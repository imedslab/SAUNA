...
class DistributedMPerClassSampler(DistributedSampler):
    ...
    def __init__(self, dataset: Dataset, labels, m, batch_size=None, num_replicas: Optional[int] = None,
        ...
        self.length_of_single_pass = self.m_per_class * \
            len(self.labels) // self.num_replicas
        ...
        if self.batch_size is None:
            if self.length_of_single_pass < self.list_size_per_proc:
                self.list_size_per_proc -= (self.list_size_per_proc) % (
    ...
    def __iter__(self):
        if self.shuffle:
            ...
            indices = torch.randperm(len(self.labels), generator=g).tolist()
        else:
        ...
        for _ in range(num_iters):
            ...
            if self.batch_size is None:
                ...
                curr_label_set = labels_per_proc[: self.batch_size //
                                                 self.m_per_class]
