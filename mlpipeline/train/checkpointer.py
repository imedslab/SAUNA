...
class Checkpointer:
    def __init__(self, pipeline):
        ...
        self.comparator = getattr(
            operator, pipeline.cfg.checkpointer.comparator)
    ...
    def save_state(self, metric_val):
        ...
        ckpt_cand_name = self.cache_dir / \
            f'epoch_{epoch:03d}_{self.model_name}_{metric_val:.4f}.pth'
        ...
        if self.best_snapshot_fname is None:
            self.logger.info(
                f'Saving the first snapshot [{os.path.abspath(ckpt_cand_name)}]')
            ...
            if self.comparator(metric_val, self.best_val_metric):
                ...
                self.logger.info(
                    f'Saving model [{os.path.abspath(ckpt_cand_name)}]')
                self.best_snapshot_fname = ckpt_cand_name
                self.best_val_metric = metric_val
                torch.save(state, ckpt_cand_name)
        return
